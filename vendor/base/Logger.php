...
class Logger
{
			...
			if (filesize($file) > $this->maxFileSize) {
				if ($this->mode === self::MODE_RENAME) {
					...
					if ($dot_pos === false) {
						...
						$filePattern = substr($file, 0, $dot_pos) . '_*';
					}
					rename($file, $new_file);
					//clear
					$this->clear($filePattern);
				}
				$fd = fopen($file, 'w');
			} else {
				$fd = fopen($file, 'a+');
	...
	public function clear($filePattern)
	{
		//获取所有当前类型的日志文件
		...
		//清除超过数量的文件
		$outnumFiles = array_slice($files, 0, -$this->keepNum);
		!$outnumFiles ?: array_map('unlink', $outnumFiles);
	}
}