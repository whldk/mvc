...
use vendor\helpers\StringHelper;
...
class Security
{
    ...
    public function hkdf($algo, $inputKey, $salt = null, $info = null, $length = 0)
    {
        ...
        $hashLength = StringHelper::byteLen($test);
    ...
    public function validateData($data, $key, $rawHash = false)
    {
        ...
        $hashLength = StringHelper::byteLen($test);
        if (StringHelper::byteLen($data) >= $hashLength) {
    ...
    public function generateRandomKey($length = 32)
    {
        ...
        if ($this->_useLibreSSL
            ...
            if ($key !== false && StringHelper::byteLen($key) === $length) {
                return $key;
            }
        }
        ...
        if (function_exists('mcrypt_create_iv')) {
            ...
            if (StringHelper::byteLen($key) === $length) {
                return $key;
            }
        }
        ...
        if (is_resource($this->_randomFile)) {
            ...
            while ($stillNeed > 0) {
                ...
                $buffer .= $someBytes;
                $stillNeed -= StringHelper::byteLen($someBytes);
                if ($stillNeed === 0) {
    ...
    public function compareString($expected, $actual)
    {
        $expected .= "\0";
        $actual .= "\0";
        $expectedLength = StringHelper::byteLen($expected);
        $actualLength = StringHelper::byteLen($actual);
