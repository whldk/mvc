...
abstract class BaseDb
{
	...
	{
		$pdo_config = array_intersect_key($config, ['dsn' => null, 'username' => null, 'password' => null]);
	...
	protected function add_prepared_param(&$val)
	{
		...
		return $val = $param_holder;
	}
	
	protected function filter_raw($val, $field = '', $raw = true)
	{
		//不是raw
		if (!$raw || is_array($raw) && !in_array($field, $raw)) {
			return $this->add_prepared_param($val);
		}
		
		if (is_array($val)) {
			throw new InvalidParamException(__METHOD__, 'val');
		}
...
// 		$pattern = '/^(distinct\s+)?([a-zA-Z0-9_]+\s*\(\s*)?([a-z0-9_]+|\*)((\.[a-z0-9_]+|\.\*)*)(\s*\)\s*)?((\s+as\s+|\s+)([a-z0-9_]+))?$/i';
	...
	public function and_filter_where($condition)
	{
		...
		if ($this->_filter_where) {
			...
			if ($op == 'AND') {
				...
				//比较运算/OR
				array_unshift($this->_filter_where, $op);
	...
	public function raw($select = false)
	{
		$db = $this->reset();
		$db->sql_type = $select ? 'raw select' : 'raw';
		return $db;
	}
	...
	{
		array_walk($vals, [$this, 'add_prepared_param']);
	}
	...
	public function select($fields = '*')
	{
		...
		if ($fields == '*') {
			...
			$db->_select = $db->comma_fields($fields);
		}
	 ...
	 * @return Db
	 */
	public function count($field = '*', $alias = 'count', $reset = true)
	{
		$db = $reset ? $this->reset(true) : $this->_clone(false);
		
		$db->sql_type = 'select';
		
		if ($field == '*') {
			$db->_select = 'COUNT(*) as ' . $alias;
		} else {
			$field = $db->build_field(strval($field));
			$db->_select = "COUNT({$field})  as {$alias}";
		}
	...
	{
		if (!is_array($fields)) {
			throw new InvalidParamException(__METHOD__, 'fields');
		}
		
		$set = [];
		if ($raw) {
			foreach ($fields as $field => $val) {
				$val = $this->filter_raw($val, $field, $raw);
				$field = $this->build_field($field);
				$set[] = "{$field} = {$val}";
			}
		} else {
	 ...
	 * @return Db
	 */
	public function insert($vals, $fields = null, $ignore = null)
	{
		$db = $this->reset();
		
		$db->sql_type = $ignore === true ? 'insert ignore' : ($ignore === false ? 'replace' : 'insert');
		
		//设置字段
		if (empty($fields)) {
			$db->_insert_fields = null;
		} else {
			$db->_insert_fields = ' (' . $db->comma_fields($fields) . ')';
		}
		...
		if (is_string($vals)) {
			//单条
			$vals = explode(',', $vals);
			$vals = array_map([$db, 'add_prepared_param'], $vals);
			$db->_insert_data = ' VALUES (' . implode(',', $vals) . ')';
		} elseif (is_array($vals) && !empty($vals)) {
			//单条/多条
			$data = [];
			foreach ($vals as $i => $val) {
				if (!is_numeric($i) && !is_array($val)) {
					$db->_insert_fields = ' (' . $db->comma_fields(array_keys($vals)) . ')';
					$vals = array_map([$db, 'add_prepared_param'], $vals);
					$data[] = implode(',', $vals);
					break;
				}
				...
				if (is_array($val) && !empty($val)) {
					$val = array_map([$db, 'add_prepared_param'], $val);
					$data[] = implode(',', $val);
				} else {
					unset($vals[$i]);	//foreach 中进行unset
				}
			}
			$db->_insert_data = ' VALUES (' . implode('),(', $data) . ')';
		}
		
		return $db;
	}
	
	protected function _clone($clean = false)
	{
		$that = clone $this;
		return $clean ? $that->reset_data() : $that;
	}
	 ...
	 * @return Db
	 */
	protected function reset($begin = true)
	{
		if ($begin) {
			if ($this->done) {
				$db = $this->reset_data();
