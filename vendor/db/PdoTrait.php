...
trait PdoTrait
{
	...
	public function result($reset = true)
	{
		...
		if ($this->stmt) {
			...
			if ($this->stmt->closeCursor() === true && $this->stmt->execute() === true) {
				if (in_array($this->sql_type, self::selectTypes, true)) {
	...
	protected function lock_tbl()
	{
		if ($this->lock) {
			return $this->execute('LOCK TABLES ' . $this->_tbl. ' WRITE', false);
		}
		return 0;
	}
	...
	public function unlock()
	{
		if ($this->lock) {
			$this->lock = false;
			return $this->execute('UNLOCK TABLES', false);
		}
		return 0;
	}
	 ...
	 */
	public function execute($sql, $lock = null)
	{
		//锁表
		if ($lock !== false) {
			$this->lock_tbl();
